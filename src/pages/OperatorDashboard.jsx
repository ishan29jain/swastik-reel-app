...
const OperatorDashboard = () => {
  ...
  const unmarkInProgress = async (reelId) => {
    const reelRef = doc(db, "reels", reelId);
    await updateDoc(reelRef, { inProgress: false });
    fetchAssignedReels();
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {filteredReels.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {filteredReels.map((reel) => (
                <div
                  key={reel.id}
                  className={`bg-gray-800 rounded-lg border border-gray-700 shadow flex items-center justify-between p-4 cursor-pointer transition hover:border-blue-400 w-full ${reel.inProgress ? 'border-2 border-green-500' : 'border border-blue-700'}`}
                  onClick={() => {
                ...
                >
                  <div className="w-full">
                    <div className="font-semibold text-white text-lg">{reel.reelNo}</div>
                    <div className="text-gray-300 text-sm mt-1">
                      Size: {reel.size} | GSM: {reel.gsm} | Quality: {reel.quality} | Mill: {reel.mill} | Weight: {reel.weight} kg
                    </div>
                  </div>
                  {reel.inProgress ? (
                    <div className="flex gap-2 ml-4">
                      <button
                        ...
                        onClick={e => { e.stopPropagation(); setSelectedReel(reel); setShowModal(true); setIsEditMode(false); }}
                      >
                        Enter Output
                      </button>
                      <button
                        className="btn-warning px-4 py-2"
                        onClick={e => { e.stopPropagation(); unmarkInProgress(reel.id); }}
                      >
                        Cancel In Progress
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      onClick={e => { e.stopPropagation(); markInProgress(reel.id); }}
                ...
                <div key={reel.id} className="card bg-gray-800 border-gray-700">
                  <div className="card-body">
                    ...
                    <button
                      ...
                      onClick={(e) => { e.stopPropagation(); handleEdit(reel); }}
                    >
                      Edit Output
                    </button>
                  </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
            <div className="bg-gray-800 rounded-lg shadow-lg w-full max-w-lg p-6 relative">
              <button
                ...
                onClick={(e) => { e.stopPropagation(); setShowModal(false); setSelectedReel(null); setIsEditMode(false); }}
              >
              ...
              <h3 className="text-lg font-bold text-white mb-4">
                {isEditMode ? `Edit Reel No.  ${selectedReel.reelNo} Output` : `Reel ${selectedReel.reelNo} Details`}
              </h3>
        ...
        {showDetailsModal && selectedReelDetails && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-gray-800 rounded-lg p-6 w-full min-w-[600px] max-w-3xl mx-auto shadow-lg">
              <h2 className="text-xl font-bold mb-4 text-white">Reel Details</h2>
              <div className="space-y-2 text-gray-200">
                <div><span className="font-semibold">Reel No:</span> {selectedReelDetails.reelNo}</div>
                <div><span className="font-semibold">Size:</span> {selectedReelDetails.size}</div>
                <div><span className="font-semibold">GSM:</span> {selectedReelDetails.gsm}</div>
                <div><span className="font-semibold">Quality:</span> {selectedReelDetails.quality}</div>
                <div><span className="font-semibold">Weight:</span> {selectedReelDetails.weight} kg</div>
                <div><span className="font-semibold">Mill:</span> {selectedReelDetails.mill}</div>
              </div>
              <button
